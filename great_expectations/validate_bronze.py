...
def load_bronze_as_df() -> pd.DataFrame:
    dt = DeltaTable(BRONZE_PATH)
    df = dt.to_pandas()
    print(f"Loaded {len(df)} records from Bronze Delta Lake")
    return df


def run_validation():
    df = load_bronze_as_df()

    context = gx.get_context(mode="ephemeral")

    data_source = context.data_sources.add_pandas("bronze_energy_source")
    data_asset = data_source.add_dataframe_asset(name="bronze_energy_asset")
    batch_definition = data_asset.add_batch_definition_whole_dataframe("bronze_batch")

    suite = context.suites.add(gx.ExpectationSuite(name="bronze_energy_suite"))
    ...
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToBeInSet(
            column="source",
            value_set=["eia_electricity", "eia_natural_gas", "eia_renewable"]
        )
    )
    suite.add_expectation(
