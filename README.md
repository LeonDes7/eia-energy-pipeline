...
\\- `renewable\\\_share\\\_by\\\_state` -- % renewable vs fossil by state and period



\\- `price\\\_volatility\\\_index` -- rolling stddev of natural gas prices by region



\\- `demand\\\_vs\\\_generation\\\_gap` -- total vs renewable generation by state



\\- `state\\\_energy\\\_mix\\\_history` -- exposes SCD Type 2 dimension table
...
\\- \\\*\\\*SCD Type 2\\\*\\\* -- `MERGE` statement tracks state energy mix changes over time with `valid\\\_from`, `valid\\\_to`, `is\\\_current`, and `row\\\_hash` fingerprinting



\\- \\\*\\\*Streams + Tasks\\\*\\\* -- Snowflake Stream watches `energy\\\_silver` for new data, Task runs SCD merge automatically on hourly CRON schedule



\\- \\\*\\\*RBAC\\\*\\\* -- read-only `energy\\\_analyst\\\_readonly` role scoped to gold schema only
...
!\\\[dbt run](screenshots/dbt\\\_run.png)
...
!\\\[Bronze GE](screenshots/bronze\\\_ge.png)
...
!\\\[Silver GE](screenshots/silver\\\_ge.png)
...
!\\\[SCD Table](screenshots/scd\\\_table.png)
...
\\# Fill in EIA\\\_API\\\_KEY, GROQ\\\_API\\\_KEY, SNOWFLAKE\\\_ACCOUNT, SNOWFLAKE\\\_USER
...
python ingestion/fetch\\\_energy.py
...
python snowflake/load\\\_silver.py
...
python great\\\_expectations/validate\\\_bronze.py



python great\\\_expectations/validate\\\_silver.py
...
│   └── fetch\\\_energy.py          # EIA API ingestion (3 series)
...
│   └── load\\\_silver.py           # Loads Silver to Snowflake via key-pair auth
...
│   ├── validate\\\_bronze.py       # 6 contracts on raw data



│   └── validate\\\_silver.py       # 5 contracts on cleaned data
